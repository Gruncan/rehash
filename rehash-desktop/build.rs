...
fn main() {
    ...
    let shared_object_abs = fs::canonicalize(Path::new(&shared_object_target))
        .expect("Failed to canonicalize shared object target");
    ...
    if !shared_object_abs.exists() {
        panic!(
            "{} does not exist, ensure codec ({}) library is build",
            shared_object_abs.display(),
            CODEC_NAME
        );
    }
    ...
    fs::copy(&shared_object_abs, dest)
        .or_else(|e| {
            eprintln!(
                "Failed to copy {} to {}: {}",
                shared_object_abs.display(),
